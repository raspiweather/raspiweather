  <h3 class="box-title">Hourly Configuration</h3>
  <div class="box-body">
  <div class="form-group">
    <label>Services</label>
    <select multiple="" class="form-control" name="hour_servs[]">
      <?php
        if (!isset($config_array['hourly']['services'])){
          $hour_servs_arr = array();
        } else {
          $hour_servs_arr = str_replace(array( '[', ']' ), '', $config_array['hourly']['services']);
          $hour_servs_arr = explode(", ", $hour_servs_arr);
        }
        ...
        for ($b = 0; $b <= count($services_arr) - 1; $b++) {
          $match[$b] = False;
          for ($i = 0; $i <= count($hour_servs_arr) - 1; $i++) {
            if (strpos($hour_servs_arr[$i], $services_arr[$b]) !== false){ 
              $match[$b] = True;
            }}
            if ($match[$b] == 1){ 
  ...
  <div class="form-group">
    ...
    <select multiple="" class="form-control" name="hour_temp[]">
      <?php   
        $con=mysqli_connect($database['host'],$database['user'],$database['pass'],$database['name']);
        if (mysqli_connect_errno())
          { echo "Failed to connect to MySQL: " . mysqli_connect_error(); }
        $result = mysqli_query($con,"SELECT * FROM templates WHERE `interval`='hourly' AND (`type`=0 OR `type`=2 OR `type`=3)");
        $temps_arr = array();
  ...
  <div class="form-group">
    <label>Twitter</label>
    <select multiple="" class="form-control" name="hour_twit[]">
      <?php
        $result = mysqli_query($con,"SELECT * FROM templates WHERE `interval`='hourly' AND `type`=1");
        $twit_arr = array();
        while($row = mysqli_fetch_array($result))
