  <h3 class="box-title">Live Configuration</h3>
  <div class="box-body">
  <div class="form-group">
    <label>Services</label>
    <select multiple="" class="form-control" name="live_servs[]">
      <?php
        if (!isset($config_array['live']['services'])){
          $live_servs_arr = array();
        } else {
          $live_servs_arr = str_replace(array( '[', ']' ), '', $config_array['live']['services']);
          $live_servs_arr = explode(", ", $live_servs_arr);
        }
        ...
        for ($b = 0; $b <= count($services_arr) - 1; $b++) {
          $match[$b] = False;
          for ($i = 0; $i <= count($live_servs_arr) - 1; $i++) {
            if (strpos($live_servs_arr[$i], $services_arr[$b]) !== false){ 
              $match[$b] = True;
            }}
            if ($match[$b] == 1){ 
  ...
  <div class="form-group">
    ...
    <select multiple="" class="form-control" name="live_temp[]">
      <?php   
        $con=mysqli_connect($database['host'],$database['user'],$database['pass'],$database['name']);
        if (mysqli_connect_errno())
          { echo "Failed to connect to MySQL: " . mysqli_connect_error(); }
        $result = mysqli_query($con,"SELECT * FROM templates WHERE `interval`='live' AND (`type`=0 OR `type`=2 OR `type`=3)");
        $temps_arr = array();
  ...
  <div class="form-group">
    <label>Twitter</label>
    <select multiple="" class="form-control" name="live_twit[]">
      <?php
        $result = mysqli_query($con,"SELECT * FROM templates WHERE `interval`='live' AND `type`=1");
        $twit_arr = array();
        while($row = mysqli_fetch_array($result))
